...
For the exact `FLOW_09B183` WiFi-UAV/FLD profile, choose **WiFi UAV native UDP**. The app sends the native stream request to `192.168.169.1` on UDP ports `8800` and `8801`, reconstructs its JPEG fragments, and defaults to **Mirror left/right** for its camera orientation; use the live **Camera orientation** selector to correct a different mounting. The **WiFi UAV camera** selector chooses the forward or bottom/flow stream through the native ACK state; the status line reports whether each stream is available. It does not use RTSP or `ffmpeg`. Use **Stop camera** to stop its stream requests and clear the display.
