...
For the exact `FLOW_09B183` WiFi-UAV/FLD profile, choose **WiFi UAV native UDP**. The app sends the native stream request to `192.168.169.1` on UDP ports `8800` and `8801`, reconstructs its JPEG fragments, and defaults to **Mirror left/right** for its camera orientation; use the live **Camera orientation** selector to correct a different mounting. Choose **Main / forward camera** or **Bottom / optical-flow camera** before starting. The selector is reconstructed from the matching Android app native stream-request field; the status line also reports whether each camera path advertises availability. It does not use RTSP or `ffmpeg`. With **Bottom / optical-flow camera** selected, the HUD reports camera-image drift in pixels per second and a confidence estimate. By default this remains visual-only. **Experimental visual drift assist** can add a small outer-loop roll/pitch correction only after you explicitly enable it, confirm a safe-hover direction calibration, hold L1, keep roll/pitch/yaw centered, and provide fresh high-confidence bottom-camera flow. It stops immediately if any gate is lost, is capped at 12% channel input, and has not been hardware validated. Start with propellers removed; then, only in a clear low hover, verify each correction direction and use the Normal/Reverse controls before checking the calibration confirmation. Use **Stop camera** to stop its stream requests and clear the display.
