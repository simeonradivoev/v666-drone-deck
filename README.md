...
For the exact `FLOW_09B183` WiFi-UAV/FLD profile, choose **WiFi UAV native UDP**. The app sends the native stream request to `192.168.169.1` on UDP ports `8800` and `8801`, reconstructs its JPEG fragments, and defaults to **Mirror left/right** for its camera orientation; use the live **Camera orientation** selector to correct a different mounting. The status line reports whether the main and flow-camera paths advertise availability; the bottom stream itself is not enabled until its native selection protocol is fully decoded. It does not use RTSP or `ffmpeg`. Use **Stop camera** to stop its stream requests and clear the display.
