...
- Incoming video-fragment header bytes `40..43` are the total compressed frame length, not an individual packet length. The native assembler copies 1,024 bytes per fragment and copies `frame_length % 1024` bytes for the final fragment (`0` means 1,024). Header bytes `44..47` supply width/height and byte `48` supplies the JPEG quality used for its generated header.
